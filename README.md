LTTN - A game consisting of endless waves of enemies and then you die.

(kinda like CoD zombies (but not all story oriented))

Built with Unreal Engine 5.7 (movement influenced heavily by epics GASP)

- Mostly C++
- Upgrade 'Weapon', Health, Stamina, etc..
- Bot pooling
- Fully replicated
- Mostly focussed on exploring gameplay mechanics
- 

Next:
- gameover/ spectate
- UI
- health/ damage component-> shared between ai and player
- level design / blockouts
- assets and stuff
- 
